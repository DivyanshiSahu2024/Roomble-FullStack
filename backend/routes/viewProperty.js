...
// Helper function to convert latitude/longitude to DMS (Degrees, Minutes, Seconds) format
...
router.post('/get_property', async (req, res) => {
    try {
        const property_id = req.body.id; // Extract property ID from the request body
        const property = await Property.findById(property_id); // Fetch property details from the database
